...
const PostListingClient = ({posts, locale, sitemap, isPreview}: Props) => {
	...
	const fetchPosts = async () => {
		...
		const morePosts = await fetch(
			`/api/get-post-listing?locale=${locale}&sitemap=${sitemap}&skip=${items.length}&isPreview=${isPreview}&take=10`
		)
		if (morePosts.ok) {
